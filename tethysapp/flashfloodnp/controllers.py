...
def getLatestForecastDate():
    try:
        conn = psycopg2.connect(host="192.168.10.163", database="GEOGloWS_V2_HIWAT", user="postgres", password="Changeit3#")
        cur = conn.cursor()
        cur.execute('select max(rundate) from public.forecastnepal')
        res = cur.fetchone()[0]
        formatted_date = res.strftime('%Y-%b-%d')
    except Exception:
        runDate = dt.datetime.now().date() - timedelta(1)
        formatted_date = runDate.strftime('%Y-%b-%d')
    finally:
...
#    print(getjson)
    forecast_run_date = getLatestForecastDate()
    context = {
        # 'myJson': getjson,
